...
def test_public_report_refuses_event_values_until_source_rights_are_signed() -> None:
    ...
    assert state["historical_intelligence"]["capability_state"] == ("released_v1_bounded")
    ...
    assert not any(
        row["computed_state"] == "operational" for row in report["capability_attestations"]
    )


def test_layer_registry_requires_truthful_publication_and_no_world_score() -> None:
    registry = json.loads(
        (ROOT / "governance" / "global_atlas_layers.json").read_text(encoding="utf-8")
    )
...
def test_unpublished_layer_cannot_smuggle_a_source_payload(
    ...
    row = next(
        item for item in registry["layers"] if not item["current_state"].startswith("published_")
    )
...
def test_hourly_audit_is_read_only_and_separates_live_health() -> None:
    audit = evolution_engine.build_runtime_audit(datetime(2026, 8, 9, 12, 0, tzinfo=timezone.utc))
    freshness = json.loads(evolution_engine.FRESHNESS_PATH.read_text(encoding="utf-8"))
    ...
    assert audit["current_freshness_ledger"]["payloads"] == len(freshness["payloads"])
...
def test_released_observer_and_world_matrix_are_not_reported_as_pending() -> None:
    registry = json.loads(evolution_engine.ENGINE_PATH.read_text(encoding="utf-8"))
    assert registry["current_automation_boundary"]["hourly_observer"] == ("live_read_only")
    report = evolution_engine.build_report()
    world = next(
...
def test_evolution_refuses_a_historical_capability_claim_without_exact_evidence(
    ...
    payload = json.loads(evolution_engine.HISTORICAL_INTELLIGENCE_PATH.read_text(encoding="utf-8"))
...
def test_hourly_workflow_observes_without_repository_authority() -> None:
    workflow = (ROOT / ".github" / "workflows" / "evolution.yml").read_text(encoding="utf-8")
...
def test_launch_contract_is_a_floor_and_history_is_an_active_program() -> None:
    ...
    design = (ROOT / "design" / "continuous_evolution.md").read_text(encoding="utf-8").lower()
...
def test_category_architecture_and_independent_review_are_governing_surfaces() -> None:
    architecture = (
        (ROOT / "design" / "category_architecture.md").read_text(encoding="utf-8").lower()
    )
    review = (
        (ROOT / "design" / "claude_adversarial_review_protocol.md")
    ...
    assert (
        "unique events, active episodes and event observations as different counts" in architecture
    )
