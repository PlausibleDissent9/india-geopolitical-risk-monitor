...
# Verified against pristine origin/main on 2026-08-11: these four carried all
# 13 pre-existing mismatches, and that baseline is what proved the perf-cascade
# work had introduced exactly one regression of its own.
FROZEN_OWNERS: dict[str, str] = {
    ...
    "validation/consequence_plan/replay_fixture/knowledge/ledger.json":
        "a replay fixture: pins the governance world its replay assumes, "
        "which must not move when the live registries do",
...
def _resolve(owner: Path, target: str) -> Path | None:
    """Find the file a pin names: repo-relative, or relative to some ancestor
    of the owner (a self-contained fixture pins its members relative to the
    fixture root). Returns None if no base resolves it."""
